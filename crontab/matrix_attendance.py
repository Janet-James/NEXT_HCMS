#!/usr/bin/python2.7
#
# Small script to show nextQL and Pyscopg together
#
    ...
    def run_attendance_tracking(self):
        try:
            ...
            for attendance in root.findall('attendance-daily'):
                ...
                if matrixUserId:
                    cr.execute('select id,org_id_id,is_active,name from employee_info where file_no=%s and is_active=True',(str(matrixUserId),))
                    res=cr.fetchall()
                    if res:
                        employee_id = res[0][0]
                        if employee_id:
...
if __name__ == '__main__':
   logging.basicConfig(filename='/home/next/logger/matrix_attendance.log', level=logging.INFO)
