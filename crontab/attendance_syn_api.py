...
class attendanceAPI():
    def run_attendance_api(self):
        try:
		conn = psycopg2.connect(database="HR_02", user = "next", password = "next", host = "192.168.10.182", port = "5432")
		cur = conn.cursor()
		cur.execute('''
		...
		from hr_attendance where employee_id_id not in (1,214) and id>=83784 and id<83785 order by id
		''')
