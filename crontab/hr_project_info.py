...
class HrProjectInfo():
	...
	def __init__(self):  
		...
		self.start_date = "'"+date_status_data[0][0]+"'"
		self.end_date = "'"+date_status_data[0][1]+"'"
		print "---start,end dates---",self.start_date,self.end_date
	
	#defining project info	
	def run_project_info(self):
		try:
			conn = psycopg2.connect("dbname='P5S_4' user='next' host='192.168.10.197' password='next'")
			cur = conn.cursor()
			cur.execute('''
...
if __name__ == '__main__':
	logging.basicConfig(filename='/home/next/logger/hr_project_info.log', level=logging.INFO)
