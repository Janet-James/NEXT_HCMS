...
class HCMSAuthUser():
    ...
    def run_auth_user_info(self):
        ...
        try:
            ...
            conn1 = psycopg2.connect("dbname='next_hcms_live' user='next' host='localhost' password='next'")
            cur1 = conn1.cursor()
...
(ei.name||' '||ei.last_name) as ename,
   ...
   where id = ei.parent_id::integer
            ...
            if user_data:
                insert_status = self.db_data_insert_update('next_p5s_live', 'next', 'next', '10.0.1.5', user_data, 'P5S')
                if insert_status:
     ...
     designation='{17}',date_joined='{18}',org_unit='{19}',org_unit_id='{20}',organization_name='{21}'   WHERE id={0};""".format(int(users[0]),str(users[1]),(users[2]),str(users[3]),str(users[4]), str(users[5]),str(users[6]),True,(users[7]),str(users[14]),
                    ...
                    if args[5] == "HCMS":
                            ...
                            if emp_check_data_trans:
                                ...
                                int(users[27]),int(users[28]),int(users[29]),users[7],(datetime.datetime.today().strftime('%Y-%m-%d')),int(users[0]),int(emp_id),(users[29])))
                            else:
                                print"$$$$$$$$$$$$$$$$$$",users[25]
                                cur.execute('''
