...
from Succession_Planning.succession_planning.models import SP_Transfer,SP_Promotion
...
@csrf_exempt
def PromotionRequestCRUDOperations(request): 
    ''' 
    ...
    try:
        ...
        if data:
                ...
                if not request_id: 
                            status_exists = SP_Promotion.objects.filter(emp_id=data[0]['promotion_employee'],promotion_status=False).count()
                            if status_exists > 0:
                                json_data[config.status_key] = config.record_already_referred
                            else:                                  
                                status = SP_Promotion(emp_id=data[0]['promotion_employee'],nominated_emp_id=data[0]['nominated_by'],    
                                                 req_date=data[0]['nominated_date'],org_id=data[0]['promotion_org'],org_unit_id=data[0]['promotion_org_unit'],
                                                 org_unit_div_id=data[0]['promotion_division'],role_id=data[0]['promotion_role'],created_by_id=int(uid),is_active="True")
                                status.save()
                                json_data[config.status_id] = status.id
                                json_data[config.status_key] = config.success_status
                else:                        
                            status = SP_Promotion.objects.filter(id=request_id).update(emp_id=data[0]['promotion_employee'],nominated_emp_id=data[0]['nominated_by'],
                                                 req_date=data[0]['nominated_date'],org_id=data[0]['promotion_org'],org_unit_id=data[0]['promotion_org_unit'],
                                                 org_unit_div_id=data[0]['promotion_division'],role_id=data[0]['promotion_role'],created_by_id=int(uid),is_active="True")
                            json_data[config.status_id] = request_id
                            json_data[config.status_key] = config.update_status                                                           
                logger_obj.info('Promotion Requests'+str(json_data)+"attempted by"+str(request.user.username))         
...
@csrf_exempt
def PromotionRequestDataTable(request):
    ''' 
    ...
    try:
        ...
        logger_obj.info('Promotion Request datatable function by'+str(request.user.username))    
        ...
        if filter_name is None:
            cur.execute(q.fetch_hcms_query(config.succession_planning, config.sp_promotion_request_data_table))
        else:
            query = q.fetch_hcms_query(config.succession_planning, config.sp_promotion_request_data_table)
...
@csrf_exempt
def PromotionRequestCRUDTableClick(request):
    ''' 
    ...
    json_data ={}
    try:
        logger_obj.info('Promotion datatable click function by'+str(request.user.username))
        post = request.POST            
        table_id = request.POST.get(config.table_id)
        cur = connection.cursor()
        query = q.fetch_hcms_query(config.succession_planning, config.sp_promotion_request_table_click)
