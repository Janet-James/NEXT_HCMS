...
from Succession_Planning.succession_planning.models import SP_Demotion as SD
...
# SP Demotion Process details
def spDemotionProcess(request):
         ''' 
         11-Sep-2018 TRU To SP Demotion Process report Search data loaded.
         ...
         try:   
                ...
                to_char(request_date,'dd-mm-yyyy') as resi_date,
                ...
                case when demotion_status = true then 'Approved' else 'Not Approved' end as rel_status,
                to_char(request_date,'yyyy-mm-dd') as start_date
                from sp_demotion_details where is_active
                """
                ...
                (select name||' '||last_name as title from employee_info where id=emp_id) as title from sp_demotion_details order by created_datatime desc
                """
                cur.execute(log_query)   
                log_details = q.dictfetchall(cur)  
                json_data['log_details'] = log_details
                cur.execute(query + ' and dep_appr_status=False and hr_appr_status=False and demotion_status=False and dep_status=False')   
                ...
                cur.execute(query + ' and dep_appr_status and hr_appr_status=False and demotion_status=False and dep_status and hr_status=False')   
                em_hrapproval_data = q.dictfetchall(cur)    
                json_data['sp_hrapproval'] = em_hrapproval_data
                cur.execute(query + ' and dep_appr_status and hr_appr_status and demotion_status and dep_status and hr_status')   
                em_exit_data = q.dictfetchall(cur)    
                json_data['sp_exit'] = em_exit_data
                cur.execute(query + ' and (dep_appr_status=False and demotion_status=False and dep_status and hr_status=False) or (hr_appr_status=False and demotion_status=False and dep_status and hr_status)')   
                em_reject_data = q.dictfetchall(cur)    
                json_data['sp_reject'] = em_reject_data
                print "----------------json_data------------",json_data
                logger_obj.info('SP Demotion Process Report search response is' + str(json_data) + "attempted by" + str(request.user.username))   
...
# SP Demotion Process event details
def spDemotionProcessEvent(request):
         ''' 
         11-Sep-2018 TRU To SP Demotion Process event report Search data loaded.
         @param request: Request Object
         @type request : Object
         @return:   HttpResponse Interview Search Data
         @author: TRU
         '''
         logger_obj.info('SP Demotion Process Report search function by' + str(request.user.username))
         json_data = {}
         post = request.GET
         ...
         try:
             query = """select a.sp_id as sp_id,
                ...
                to_char(a.request_date,'dd-mm-yyyy') as res_date,coalesce(a.emp_notes,'') as emp_remarks,
                case when dep_appr_status = true then 'Approved' else 'Not Approved' end as dep_status ,
                case when hr_appr_status = true then 'Approved' else 'Not Approved' end as hr_status,
                case when demotion_status = true then 'Approved' else 'Not Approved' end as rel_status,
                to_char(a.request_date,'dd-mm-yyyy') as exp_res_date,
                ...
                coalesce(a.std_role_id,0) as role_id,
                ...
                coalesce((select role_title from hcms_ti_role_details where id = a.ei_role_id),'') old_role,
                coalesce((select role_title from hcms_ti_role_details where id = a.std_role_id),'') change_role
                from(select std.id sp_id,ei.id sp_emp_id,std.org_id std_org_id,std.org_unit_id std_org_unit_id,std.org_unit_div_id std_org_unit_div_id,
                std.role_id std_role_id,ei.role_id_id ei_role_id,* from sp_demotion_details std
             ...
             logger_obj.info('SP Demotion Process search response is' + str(json_data) + "attempted by" + str(request.user.username))   
         except Exception as e:
              print e
              logger_obj.error(e)
              json_data['error'] = []
...
def spProcessDemotionEventUpdate(request):
         ''' 
         11-Sep-2018 TRU To SP Demotion Process event Update report Search data loaded.
         @param request: Request Object
         @type request : Object
         @return:   HttpResponse Interview Search Data
         @author: TRU
         ...
         try:
             if status == 'department':
                 status = SD.objects.filter(id=update_id).update(dep_status=True,dep_appr_status=dep_status,dep_notes=str(post.get('dep_remark')),dep_emp_id=int(post.get('dep_change_by')),role_id=int(post.get('dep_change_role_id')),is_active="True",modified_by_id=int(uid))
                 json_data['results'] = '1'
             elif status == 'hr':
                 status = SD.objects.filter(id=update_id).update(hr_status=True,hr_appr_status=dep_status,demotion_status=dep_status,hr_notes=str(post.get('hr_remark')),hr_emp_id=int(post.get('hr_change_by')),role_id=int(post.get('hr_change_role_id')),is_active="True",modified_by_id=int(uid))
                 json_data['results'] = '2'
