...
@csrf_exempt
def HCMSBankBasedEmployee(request): # employee details create function
    ''' 
    ...
    '''
    #try:
    print '$$$$$$$$$$$$$$$$$$44',request.POST.get('banking_type')
    cur = connection.cursor() 
    json_data = {}
    bank_type= request.POST.get('banking_type')
    if bank_type == "sbi":
    	cur.execute("""Select pa.employee_id_id as id from hr_payment_advices pa left join employee_info emp on pa.employee_id_id=emp.id where pa.bank_name_id = 151 and pa.is_active and emp.is_active""")
...
@csrf_exempt
def HRMSBankFormat(request): # employee details create function
    ''' 
    26-May-2018 VJY To HRMS Create Salary Rule function. 
    @param request: Request Object
    @type request : Object
    ...
    banking_payer = request.POST.get('banking_payer')
    banking_type = request.POST.get('banking_type')
    print"VVVVVVVVV",banking_payer,banking_type
    if from_date:
    ...
    print"DDDDDDD",pay_month,split_year
    ...
    if data_value :
        ...
        if to_date:
            ...
            end_date = ''
        if banking_payer:
	    cur.execute("""select account_no,ifsc_code,branch_name,branch_code from bank_type where id=%s and is_active""" , (banking_payer,))
            payer_details = q.dictfetchall(cur)
	    if payer_details:
		payer_ifsc_code = payer_details[0]['ifsc_code']
		payer_account_no = payer_details[0]['account_no']
		payer_branch_name = payer_details[0]['branch_name']
		payer_branch_code = payer_details[0]['branch_code']
        if data_val:
...
#                                 hr_hour_val = datetime.datetime.strptime('08:00:00', '%H:%M:%S')
        ...
        #                         check_in_time = datetime.datetime.strptime(i['check_in_time'], '%H:%M:%S')
		    ...
		    ctc_amount = float(base_amount[0]['base_amount'])
               ...
               cur.execute("""select emp.id as emp_id,COALESCE(concat(upper(emp.name), ' ',upper(emp.last_name)),'') as employee_name,emp.employee_id as employee_id,
                    team.name as team_name,tirole.role_title,to_char(emp.date_of_joining,'DD/MM/YYYY') as date_of_joining,
                    hr_payment_advices.account_no,hr_payment_advices.ifsc_code,hr_payment_advices.branch_code,refitems_name as bank_name,hr_payment_advices.branch_name
...
#                                    current_date = datetime.datetime.today().strftime("%d/%m/%Y")
            ...
            if result_data:
               result = csv_generate(request,result_data,worked_day,working_days,lop_days,payer_ifsc_code,payer_account_no,payer_branch_name,payer_branch_code,banking_type,pay_month,split_year)
...
def csv_generate(request,dict_value,worked_day,working_days,lop,payer_ifsc_code,payer_account_no,payer_branch_name,payer_branch_code,banking_type,pay_month,pay_year):
    cr = connection.cursor()
    report_date = datetime.datetime.now()
    file_date = report_date.strftime("%Y-%m-%d %H:%m:%S.%f")[:-3]
    sbi_date = report_date.strftime("%d/%m/%Y")
    non_sbi_date = report_date.strftime("%d-%m-%Y")
    print"DFFFFFFFFFF",file_date
    random =report_date.strftime("%Y%m%d") 
    uid = request.user.id
    if not uid:
        uid = 1
    month = calendar.month_name[pay_month]
    data = [
    ...
    for j in dict_value['exp_details']:
	    ...
	    if j['employee_data_val']:
		    ...
		    if banking_type == 'sbi':
                    	...
                    	overall_data.append(sbi_date)
			overall_data.append(None)
			overall_data.append(net_salary)
                    	overall_data.append(random + str(l['emp_id'])) 
		    	overall_data.append('Salary For '+month+' '+ str(pay_year))
                    else:
			overall_data.append(l['account_no'])
                    	overall_data.append(l['ifsc_code'])
                    	overall_data.append(non_sbi_date)
			overall_data.append(None)
                    	overall_data.append(net_salary)
                    	overall_data.append(random + str(l['emp_id'])) 
		    	overall_data.append('Salary For '+month+' '+ str(pay_year))
			overall_data.append('NEFT')
    ...
    if banking_type == 'sbi':
    	header = [payer_account_no, payer_branch_code, sbi_date,total_net_salary,None,random+'00','Salary For '+month+ ' '+ str(pay_year)]
    else:
    	header = [payer_account_no, payer_branch_code, non_sbi_date,total_net_salary,None,random+'001','Salary For '+month+ ' '+ str(pay_year),'NEFT']
