...
function generateExpertise(){
	...
	if(form_status){
		...
		$.ajax({
			url : "/exit_employee_details_operation/",
			type : "POST",
			...
			if( res_data == 'NTE-E01'){
				alert_lobibox("success", "Exit Expertise Added Successfully.");
				employeeOfferList();
				clearExpertise();
			}else if( res_data == 'NTE-E02'){
				alert_lobibox("success", "Exit Expertise Updated Successfully.");
				employeeOfferList();
				clearExpertise();
			}else{
				alert_lobibox("error", "Exit Expertise Added Error.");
				clearExpertise();
			}
		});
	}else{
