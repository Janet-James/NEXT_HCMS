...
function searchDatas(){
	...
	if(status == 'NTE-TS' ){
		...
		if(orgz_id != '0' && orgz_unit_id != '0' && tbl_status != ''){
			if(e_fname_status !='' || e_lname_status != '' || e_gname_status != '' || id_no_status != ''){
				datas = {'tbl_status':tbl_status,'f_name':e_fname_status,'l_name':e_lname_status,'g_name':e_gname_status,'id_no':id_no_status,"org_ids":orgz_id,"orgz_unit_id":orgz_unit_id}
				datTableFunction(datas);
			}else{
				swal({
		...
		async:false,
			...
			function(json_data)
			{
				...
				if(tbl_status == 'NTE-MUL'){
					plaindatatable_btn('attendance_popup_table',data.results,columnDefsMulti,0);
				}else{
					plaindatatable_btn('attendance_popup_table',data.results,columnDefsSingle,0);
				}
			});
}
