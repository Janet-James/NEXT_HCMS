...
(function($) {
    ...
    var getMonthHtml = function(month, events) {
        ...
        $('.calendar-actions', header).append('<button type="button" class="prev btn btn-success btn-eql-wid btn-animate ">Previous</button> ');
        $('.calendar-actions', header).append('<button type="button" class="next btn btn-success btn-eql-wid btn-animate ">Next</button> ');
        $('.calendar-actions', header).append('<button type="button" class="today btn btn-warning btn-eql-wid btn-animate ">Today</button> ');
    ...
    var getDateHtml = function(date, events, reservedSlots, activeMonth, currentDate) {
        ...
        dateHtml.append('<ul class="date-events dateEvents"></ul>');
    ...
    var getEventHtml = function(event) {
        var eventHtml = $('<a onclick="calendarEvent(\'' + event.getid + '\',\'' + event.key3 + '\')" class="event ' + event.class + '"  id="' + event.getid + '"  href="' + event.url + '" ' + (event.color != '' ? 'style="border-color: ' + event.color + ';"' : '') + '></a>');
        eventHtml.data('event-id', event.id);
        eventHtml.append('<span class="key1">' + event.key1 + '</span>');
        eventHtml.append('<b><span class="key2">' + event.key2 + '</b></span>');
        return eventHtml;
