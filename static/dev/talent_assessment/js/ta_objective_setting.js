...
function editable_datatable(tbl_name,tbl_plus_id, titles,data) {
    ...
    var oTable = table.dataTable({
        ...
        "pageLength": 5,
        "language": {
        "lengthMenu": " _MENU_ records","zeroRecords": "No data available",
 	   "search": ""},
        "columns": titles,
        "data": data,
        "buttons":[],
...
//    } );
/*    jQuery('#role_table_id').wrap('<div class="dataTables_scroll" />');
*/    role_table_data();
});
...
function kpi_datatable(){
    var columns = [{"title":"Summary"}, {"title":"Target"}, {"title":"Target Type"},
...
//function for assessment form
$("#assessment_setting_submit").click(function(){
	    data = {}
	    var objective_kpi_empty_flag=''
	    var kpi_action_item_list = []
...
//	        	alert_status(data.status)
	            ...
	            if(data.kpi_id)
	            {
	           	 objective_kpi(data.kpi_id);
	             clear_objectives();
//	             $('#objective_setting_popup').hide()
	            }
	            }
	         else if (data.status=='Fill all KPI value') {alert_lobibox("error",sysparam_datas_list["ERR0033"])}
	         else if (data.status=='Start Date should greater than End Date Format') {alert_lobibox("error",sysparam_datas_list["NTE_21"])}
...
{   
	$("#obj_setting_btn").empty();
	if (flag=='add')
	{ 
		var btnstr = '<button type="submit" class="btn btn-success btn-eql-wid btn-animate margin-right-3" id="objective_submit">Add</button>';
	}
	else
	{  

		var btnstr= '<button type="button" class="btn btn-primary btn-eql-wid btn-animate margin-right-3" id="objective_update"><i class="fa fa-green"></i> Update</button>';
