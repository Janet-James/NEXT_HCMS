...
function dbListViewTb(data,divId,key){
	if(data.length > 0){
		...
		for(var i=0; i<data.length; i++){
			...
			list_content += '<div class="col-sm-3" data-toggle="tooltip" data-placement="left" title="Promotion Date">'+data[i].resi_date+'</div>';
...
function tablistClickEvent(getID,divId){
	...
	if(data.length > 0){
		...
		for(var i=0; i<data.length; i++){
			...
			content += '<h2>Exp. Promotion Date</h2>'+'<p>'+data[i].res_date+'</p>';
			content += '<div id="readmore'+divId+'" class="readMore" style="display:none">';//read mored content here start
			content += '<h2>Employee Promotion Organization Unit </h2>'+'<p>'+data[i].torg_unit_name+'</p>';
			content += '<h2>Employee Promotion Organization Divison </h2>'+'<p>'+data[i].tdivison_name+'</p>'; 
...
function departmentUpdate(){
	...
	if(( dep_app_status || dep_rej_status )){
		if( status ){
			...
			let emp_id = getEmployeeId(getUserID,'emp_id','sp_promotion_details');//get employee id 
...
function updateStageApproval(datas){
	$.ajax({
		url : "/sp_promotion_process_update/",
		type : "POST",
		data : datas,
		timeout : 10000,
		async:false,
...
function listViewEvent(putDatas){
	...
	$.ajax({ 
		url : '/sp_promotion_process_event_details/',
		type : 'GET',
		data : putDatas, 
		timeout : 10000,
		async : false,
...
function dbEMProcessDetails(){
	var data;
	$.ajax({ 
		url : '/sp_promotion_process_details/',
		type : 'GET',
		timeout : 10000,
		async : false,
	}).done(function(json_data){ 
	...
	if(data.length>0){
		...
		for(var i=0 ; i < data.length; i++){
			cal_datas.push({
				...
				key2: 'Promotion Status : '+data[i].rel_status,
				url: '#',
				class: 'events',
				color: '#000',
				getid: data[i].id
