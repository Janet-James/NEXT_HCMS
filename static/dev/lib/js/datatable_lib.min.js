...
function plaindatatable_btn(tbl_name, data_list, tbl_columns, hidden_col,filename) {
	rowLenth=data_list.length;
	let excel_filename = filename;
	...
	table.dataTable({
		...
		language: {
			...
			infoFiltered: "(Filtered _MAX_ entries)",
		...
		buttons: [{
			...
			buttons: [{
				...
				customize: function (doc) { pageNumberPDF(doc,tbl_columns) }
			}, {
				extend: "excel",
				className: "excelicon",
				title: filename,
				...
				customize: function(xlsx) { pageHeaderExcel(xlsx,table.fnGetData().length);
		              ...
		              source.setAttribute('name',excel_filename);
				}
			}, {
				extend: "csv",
				className: "csvicon",
				...
				customize: function (csv) {
					return "Export Date : "+jsDate+" \nOrganization : NEXT Inc.\nLocation : Coimbatore.\n\n"+  csv+"\n\n\tTotal Number of Records : "+table.fnGetData().length;
				}
			}],
		},{
			extend: "print",
...
/* PAR*/
	try{
		$('#'+tbl_name).on('search.dt', function() {
		    let value = $('.dataTables_filter input').val();
		    if(value){
		    
		    	$('#'+tbl_name+'_length').hide();
		    }
		    else{
		    	$('#'+tbl_name+'_length').show();
		    }
		});		
	}
	catch(e){
...
function  pageHeaderExcel(xlsx,tbl_len) {
	...
	var r4 = Addrow(tbl_len+7,[{
		k: 'B',
		v: 'Total Number of Records : '+tbl_len
	},{
		k: 'C',
		v: ''
	}])
...
function pdfColWidth(colLen){
	let arrWidth = [];
	let len = colLen.length;
...
function pageNumberPDF(doc,elLenth){
	doc.content.splice(0,1);
	let colmWidth = pdfColWidth(elLenth)
	doc.content.forEach(function(item) {
        if (item.table) {
           item.table.widths = colmWidth;
        }
     });
	...
	let count = doc.content[0].table.body.length-1;
	let total_rocords = 'Total Number of Records : '+count;
	doc.content[0].table.body.push([{text: total_rocords,style: 'tableBodyOdd'},{text:'',style: 'tableBodyOdd'},{text: '',style: 'tableBodyOdd'},{text: '',style: 'tableBodyOdd'},{text: '',style: 'tableBodyOdd'},{text: '',style: 'tableBodyOdd'}]);
}
...
function editable_datatable(tbl_name, data_list, tbl_plus_id, titles) {
...
$("#table_view").click(function(){
	$('#ta_open_main_div1').removeClass('col-md-12 col-md-8')
	$('#ta_open_main_div2').removeClass('col-md-4 divActive');
	$('#ta_open_main_div1,#ta_open_main_div2,#table_view').css('display','none');
...
$("#form_view").click(function(){
	...
	$('#ta_open_main_div1,#ta_open_main_div2,#table_view').css('display','block');
	$('#ta_main_div2').removeClass('col-md-12');
	$('#ta_open_main_div1').addClass('col-md-12');
	$('#ta_open_main_div2').addClass('col-md-4').css('display','none');
	$('#right').trigger('click');
});

//10-Jan-2018 || TRU || Update Restriction
function UpdateReistriction(x,y){
...
//datatable trigger to close
function triggerToClose(){
	$('.dt-button-background').trigger('click');
}

//21-Jan-2018 || TRU || GET Employee Id
function getEmployeeId(id,field,tble){
	let get_datas = { 'id':id,'field':field,'table':tble };
	let datas = [];
	$.ajax({ 
		url : '/get_employee_id/',
		type : 'GET',
		...
		let get_datas = JSON.parse(json_data);
		datas = [ get_datas.datas[0].emp_id ]
	});  
	return datas
}

function datatable_with_export(tbl_name, data_list, tbl_columns, hidden_col, filename, sum_col) {
	rowLenth=data_list.length;
	let excel_filename = filename;
	if ($.fn.DataTable.isDataTable('#'+tbl_name)) {
		$('#'+tbl_name).DataTable().destroy();
	...
	table.dataTable({
		...
		buttons: [{
			...
			buttons: [{
				...
				customize: function (csv) {
					return "Export Date : "+jsDate+" \nOrganization : NEXT Inc.\nLocation : Coimbatore.\n\n"+  csv+"\n\n\tTotal Number of Records : "+rowLenth;
				}
			}],
		},{
			extend: "print",
