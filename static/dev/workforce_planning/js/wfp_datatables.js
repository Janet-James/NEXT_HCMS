...
var data_list_len='';
var hidden_col_colvis='';

//Empty Datatable Load function
function empty_datatable(tbl_name,sum_col,hidden_col){
	var table = $("#"+tbl_name);
	var oTable = table.DataTable({
		"footerCallback": function ( row, data, start, end, display ) {
			...
			for(i=0; i<sum_col.length; i++){
				// Total over all pages
				this['total'+sum_col[i]] = api
				.column( sum_col[i] )
				.data()
				.reduce( function (a, b) {
				...
				this['pageTotal'+sum_col[i]] = api
				.column( sum_col[i], { page: 'current'} )
				.data()
				.reduce( function (a, b) {
					return intVal(a) + intVal(b);
				}, 0 );
				...
				// Update footer
				$( api.column( sum_col[i] ).footer() ).html( this['pageTotal'+sum_col[i]] +' ('+ this['total'+sum_col[i]] +' total)');
			}
		},
...
function wfp_editable_datatable(tbl_name, data_list, tbl_plus_id, titles) {
	// Editable Table load
	var table = $("#"+tbl_name);
	var oTable = table.dataTable({
...
//Plain Datatable with Export Buttons
function plain_datatable_with_export(tbl_name, data_list, tbl_columns, sum_col, hidden_col) {
	data_list_len=data_list.length;
	if ($.fn.DataTable.isDataTable('#'+tbl_name)) {
		$('#'+tbl_name).DataTable().destroy();
	}
	if (hidden_col != undefined){
	...
	table.dataTable({
		...
		buttons: [{
			...
			filename:  'NTE_'+now_date+'_WorkforcePlanning',
			text: 'Export',
			exportOptions: {
				columns: ':visible'
			},
			buttons: [{
				...
				className: "pdficon",
				filename:  'NTE_'+now_date+'_WorkforcePlanning',
				messageTop: '<p><b>Workforce Planning</b></p>',
			...
			},
			{
				extend: "excel",
				className: "excelicon",
				messageTop: '<p><b>NTE_'+now_date+'_WorkforcePlanning</b></p>',
				filename:  'NTE_'+now_date+'_WorkforcePlanning',
	...
	doc['header']=(function() {
		return {
			...
			},
			{
				alignment: 'center',
				fontSize: 16,
				text: 'NTE_'+now_date+'_WorkforcePlanning',
				margin: [0, 15, 20, 0]
			}
			],
		}
...
function  pageHeaderExcelRole(xlsx) {
