...
getOrgChart.prototype.showGridView = function() {
    var a = '<table id="getOrgChartExport" class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" cellpadding="0" cellspacing="0" border="0">';
    a += "<thead><tr>";
    for (i = 0; i < this._aj.length-1; i++) {
        var c = this._aj[i];
        	a += "<th>" + c + "</th>"
    }
    a += "</tr></thead>";
...
function exportTable(){
		...
		var header = function (pdf, pageCount, options) {
			pdf.setFontSize(10);
	    ...
	    function (dispose) {
	        ...
	        pdf.save('NTE_'+day+'-'+month+'-'+year+'_HCMS_'+'orgchart_datas.pdf');
	    }, margins);
}
